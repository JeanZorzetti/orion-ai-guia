...
app = FastAPI(
    title="Orion ERP API - Production Ready",
    version="1.0.0",
    description="Orion ERP API para produção com todos os endpoints funcionais"
)
...
@app.get("/")
async def root():
    return {"message": "Orion ERP API is running - PRODUCTION", "version": "1.0.0", "status": "healthy"}
