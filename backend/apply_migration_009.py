"""
Script para aplicar migration_009_woocommerce.sql no banco de dados de produção
...
def apply_migration():
    """Aplica a migration 009 (WooCommerce) no banco de dados"""

    print("=" * 60)
    print("APLICANDO MIGRATION 009: WooCommerce Integration")
    print("=" * 60)
    print()
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(text("""
                ...
                AND column_name LIKE 'integration_woocommerce%'
                ORDER BY column_name;
            """))

            columns = result.fetchall()
            if columns:
                print(f"✅ {len(columns)} coluna(s) WooCommerce encontrada(s):")
                for col in columns:
                    print(f"   - {col[0]} ({col[1]}{f'({col[2]})' if col[2] else ''})")
            else:
                print("⚠️  Nenhuma coluna WooCommerce encontrada (pode ser um erro)")

            print()
            print("=" * 60)
            print("MIGRATION 009 CONCLUÍDA")
            print("=" * 60)
            print()
            print("Próximos passos:")
            print("1. Reinicie o backend (se estiver rodando)")
            print("2. Acesse /admin/integracoes no frontend")
            print("3. Configure o WooCommerce com suas credenciais")
            print()
