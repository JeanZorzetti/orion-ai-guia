...
export const FinancialKPIs: React.FC = () => {
  const { kpis, loading, error } = useFinancialKPIs(30); // 30 dias de período
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-center h-[200px]">
            {error ? (
              <p className="text-red-600">Erro ao carregar KPIs: {error}</p>
            ) : (
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {kpis.liquidezImediata !== undefined && (
            ...
            {kpis.liquidezCorrente !== undefined && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {kpis.cicloFinanceiro !== undefined && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {kpis.endividamentoTotal !== undefined && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">Endividamento</span>
                  <Badge variant={kpis.endividamentoTotal <= 50 ? 'default' : 'destructive'}>
                    {kpis.endividamentoTotal <= 50 ? 'Controlado' : 'Alto'}
                  </Badge>
                </div>
                <div className={`text-3xl font-bold ${kpis.endividamentoTotal <= 50 ? 'text-green-600' : 'text-red-600'}`}>
                  {kpis.endividamentoTotal.toFixed(1)}%
                </div>
                <Progress value={Math.min(100, kpis.endividamentoTotal)} className="h-2" />
                <p className="text-xs text-muted-foreground">
